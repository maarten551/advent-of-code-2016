...
let nameToId = {};
let rememberCurrentAmountOfSteps = 0;
...
function parseStringToFloor(floorId, nonParsedFloor) {
    ...
    if (chipParseResults != null) {
        chipParseResults.forEach(chipName => {
            ...
            if(!(chipName in nameToId)) {
                nameToId[chipName] = Object.keys(nameToId).length;
            }

            floor.addMoveAble(new MicroChip(entityIdentifierCounter++, chipName));
        });
    }
...
class Floor {
    ...
    getMoveAbleIdentifiersInOrder() {
        let couples = this.microchips.filter(m => this.generators.some(g => g.name == m.name)).map(m => nameToId[m.name]);

        return [
            ...this.generators.filter(g => !couples.some(c => nameToId[g.name] == c)).map(generator => nameToId[generator.name]),
            ...this.microchips.filter(m => !couples.some(c => nameToId[m.name] == c)).map(microchip => nameToId[microchip.name]),
            ...couples.map(c => -1)
        ].sort();
    }
...
class Environment {
    ...
    bruteForceBestOption() {
        ...
        while (queue.length > 0) {
            ...
            if (environment.currentAmountOfSteps > rememberCurrentAmountOfSteps) {
                rememberCurrentAmountOfSteps = environment.currentAmountOfSteps;
                console.log(rememberCurrentAmountOfSteps + " - " + queue.length + " : " + (new Date - startDate) / 1000);
            }
    ...
    iterateCurrentEnvironment(queue) {
        ...
        let startElevatorPointer = (this.areFloorsBeneathEmpty()) ? 1 : -1;

        for (let i = startElevatorPointer; i <= 1; i += 2) {
            if ((this.elevatorLevel + i) in this.floors) {
                ...
                for (let i = 0; i < currentFloor.generators.length; i++) {
                    this.createNewPossibleEnvironment(queue, currentFloor, nextFloor, currentFloor.generators[i], null);
                    for (let j = i + 1; j < currentFloor.generators.length; j++) {
                        this.createNewPossibleEnvironment(queue, currentFloor, nextFloor, currentFloor.generators[i], currentFloor.generators[j]);
                    }

                    for (let j = 0; j < currentFloor.microchips.length; j++) {
                        this.createNewPossibleEnvironment(queue, currentFloor, nextFloor, currentFloor.generators[i], currentFloor.microchips[j]);
                    }
                }

                for (let i = 0; i < currentFloor.microchips.length; i++) {
                    this.createNewPossibleEnvironment(queue, currentFloor, nextFloor, currentFloor.microchips[i], null);
                    for (let j = i + 1; j < currentFloor.microchips.length; j++) {
                        this.createNewPossibleEnvironment(queue, currentFloor, nextFloor, currentFloor.microchips[i], currentFloor.microchips[j]);
                    }
                }
            }
        }
    ...
    areFloorsBeneathEmpty() {
        for(let i = 1; i < this.elevatorLevel; i++) {
            if (!this.floors[i].isEmpty()) {
                return false;
            }
        }
